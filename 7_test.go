...
func testDay7(t *testing.T, tc []StringIntTC, phases []int, outputFunc func([]int, []int) int) {
	for _, c := range tc {
		actual := maxAmplifiersOutput(strings.NewReader(c.Input), phases, outputFunc)
		if actual != c.Expected {
			t.Errorf("Expected max amplifier output to be %d, got %d", c.Expected, actual)
		}
	}
}
...
func Test7a(t *testing.T) {
	...
	input, err := os.Open(filepath.Join("input", "7"))
	aocutil.Check(err)
	defer input.Close()
