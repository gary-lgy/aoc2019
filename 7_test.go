...
func Test7a(t *testing.T) {
	...
	input := testutil.ReadStringFromFile(t, filepath.Join("input", "7.txt"))
	tc = append(tc, stringIntTC{Input: input, Expected: 262086})
...
func Test7b(t *testing.T) {
	...
	input := testutil.ReadStringFromFile(t, filepath.Join("input", "7.txt"))
	tc = append(tc, stringIntTC{Input: input, Expected: 5371621})
